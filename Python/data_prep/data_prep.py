...
# =====================
logging.info(f"\n🔍 DEBUG - Analyse des coordonnées:")
logging.debug(f"  Locations uniques dans df: {df['location'].nunique()}")
logging.info(f"  Addresses géocodées: {len(addresses)}")
logging.debug(f"  Lignes avec lat/lng non-null: {df_enriched[['latitude', 'longitude']].dropna().shape[0]}")
...
logging.debug(df_enriched[[addr_col, "latitude", "longitude", "country", "country_iso2", "iso_3166_2"]].head())
